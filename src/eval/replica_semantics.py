...
def _render_trajectory_video(
    ...
    with ExitStack() as stack:
        ...
        with progress:
            # In this loop you can change which pictures you want to use for the evaluation
            for camera_idx in progress.track(range(0, cameras.size, 1), description=""): #range(cameras.size)
                # if camera_idx > 1:  # debug
                #   break
                ...
                img_gt_idx = str(camera_idx).zfill(5)
                image_gt_path = f'data/nerfstudio/replica_{scene}/images/frame_{img_gt_idx}.jpg'
                depth_gt_path = f'data/nerfstudio/replica_{scene}/depths/frame_{img_gt_idx}.png'
                ...
                # Keep track of assigned cathegories for later majority vote
        ...
        for idx in range(len(saga_aggregate)):
            if len(vote_counts[idx]) > 0:
                ...
                saga_aggregate[idx] = majority_class_index

        with open(output_image_dir / 'saga.npy', 'wb') as f:
            np.save(f, saga_aggregate)
        ...
        v.add_points(f'semantics_ours', points[mask], semantic_classes_pr_colors[mask], normals[mask], resolution=4)
