...
class SensorData:
    def __init__(self, filename, downsampling_factor=1):
        self.version = 4
    ...
    def export_depth_images(self, output_path, image_size=None):
        ...
        print(
            "exporting", len(self.frames) // self.downsampling_factor, " depth frames to", output_path
        )
        for f in range(0, len(self.frames), self.downsampling_factor):
            ...
            imageio.imwrite(os.path.join(output_path, str(int(f/self.downsampling_factor)) + ".png"), depth)

    def export_color_images(self, output_path, image_size=None):
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        print(
            "exporting", len(self.frames) // self.downsampling_factor, "color frames to", output_path
        )
        for f in range(0, len(self.frames), self.downsampling_factor):
            ...
            imageio.imwrite(os.path.join(output_path, str(int(f/self.downsampling_factor)) + ".jpg"), color)
    ...
    def export_poses(self, output_path):
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        print(
            "exporting", len(self.frames) // self.downsampling_factor, "camera poses to", output_path
        )
        for f in range(0, len(self.frames), self.downsampling_factor):
            self.save_mat_to_file(
                self.frames[f].camera_to_world,
                os.path.join(output_path, str(int(f/self.downsampling_factor)) + ".txt"),
            )
