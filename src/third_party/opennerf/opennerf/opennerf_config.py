...
from opennerf.data.utils.scannet_dataparser import ScanNetDataParserConfig


"""
Swap out the network config to use OpenCLIP or CLIP here.

If you are switching between REPLICA and ScanNet, you will need to change the data parser as well.
"""
    ...
    config=TrainerConfig(
        ...
        pipeline=OpenNerfPipelineConfig(
            datamanager=OpenNerfDataManagerConfig(
                # Adapt Dataparser here if necessary
                dataparser=ScanNetDataParserConfig(train_split_fraction=0.99),
