...
def _render_trajectory_video(
    ...
    with ExitStack() as stack:
        ...
        as_op_path = f'/home/luca_luis/adl4cv/decoupled_3dseg/results/pr_semantics/semantics_assignment_{scene}_openseg.txt'
        with open(as_op_path) as f:
            semantic_classes_as_op = [int(l.rstrip()) for l in f.readlines()]
            semantic_classes_as_op_colors = np.array([list(replica.SCANNET_COLOR_MAP_200.values())[i] for i in semantic_classes_as_op])
        
        as_ov_path = f'/home/luca_luis/adl4cv/decoupled_3dseg/results/pr_semantics/semantics_assignment_{scene}_ovseg.txt'
        with open(as_ov_path) as f:
            semantic_classes_as_ov = [int(l.rstrip()) for l in f.readlines()]
            semantic_classes_as_ov_colors = np.array([list(replica.SCANNET_COLOR_MAP_200.values())[i] for i in semantic_classes_as_ov])

        ma_op_path = f'/home/luca_luis/adl4cv/decoupled_3dseg/results/pr_semantics/semantics_matching_{scene}_openseg.txt'
        with open(ma_op_path) as f:
            semantic_classes_ma_op = [int(l.rstrip()) for l in f.readlines()]
            semantic_classes_ma_op_colors = np.array([list(replica.SCANNET_COLOR_MAP_200.values())[i] for i in semantic_classes_ma_op])

        ma_ov_path = f'/home/luca_luis/adl4cv/decoupled_3dseg/results/pr_semantics/semantics_matching_{scene}_ovseg.txt'
        with open(ma_ov_path) as f:
            semantic_classes_ma_ov = [int(l.rstrip()) for l in f.readlines()]
            semantic_classes_ma_ov_colors = np.array([list(replica.SCANNET_COLOR_MAP_200.values())[i] for i in semantic_classes_ma_ov])

        mask = points[:, 2] < np.max(points[:, 2]) * 0.8  # cut off the ceiling so we can easy see inside the scene
        v.add_points(f'opennerf', points[mask], semantic_classes_pr_colors[mask], normals[mask], resolution=4)
        v.add_points(f'gt', points[mask], semantic_classes_gt_colors[mask], normals[mask], resolution=4)
        v.add_points(f'as_op', points[mask], semantic_classes_as_op_colors[mask], normals[mask], resolution=4)
        v.add_points(f'as_ov', points[mask], semantic_classes_as_ov_colors[mask], normals[mask], resolution=4)
        v.add_points(f'ma_op', points[mask], semantic_classes_ma_op_colors[mask], normals[mask], resolution=4)
        v.add_points(f'ma_ov', points[mask], semantic_classes_ma_ov_colors[mask], normals[mask], resolution=4)
